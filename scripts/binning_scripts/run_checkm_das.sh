#$ -S /bin/bash
#$ -N checkm_das
#$ -e /workdir/users/agk85/CDC2/logs/das_checkm_$JOB_ID.err
#$ -o /workdir/users/agk85/CDC2/logs/das_checkm_$JOB_ID.out
#$ -t 40-43
#$ -V
...
FOLDER=$WRK/das/${NAME}/${NAME}_DASTool_bins
OUT=$WRK/das/${NAME}/checkm_lineage

 if [ ! -d $OUT ]; then mkdir -p $OUT; fi

LOG=$WRK/logs/das_checkm_$JOB_ID_${NAME}.out

echo start checkm ${NAME} `date` >> $LOG 2>&1

#run checkm
source  /programs/checkm_161011/init
checkm lineage_wf -t 16 -x fa $FOLDER $OUT
