...
d = read.csv("/workdir/users/agk85/CDC2/bins/hq_bins_mil_dist.txt", stringsAsFactors=FALSE, header= F, row.names=1, sep='\t')
...
pdf("/workdir/users/agk85/CDC2/bins/hq_bins_mil_heatmap_mash.pdf", height=100, width=100)
...
pdf("/workdir/users/agk85/CDC2/bins/hq_bins_mil_species_heatmap_mash.pdf", height=100, width=100)
...
pdf("/workdir/users/agk85/CDC2/bins/hq_bins_mil_krakencont_heatmap_mash.pdf", height=100, width=100)
...
pdf("NMDS_hq_mil_mash_class+species.pdf",height=15,width=15)
ggplot(nmds.df) +
geom_point(aes(NMDS1,NMDS2, color=factor(class)))+
  geom_text(aes(NMDS1,NMDS2,label = species),size=2)
dev.off()
