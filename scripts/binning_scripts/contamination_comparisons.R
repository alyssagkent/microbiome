...
scale_shape_manual(values = c(0,1,2,3,4,5,6,8,9))+
...
hq.metadata <-subset(metadata, Contamination<10,Rank_name="speces")

pdf("Contamination_comparison2_q_species.pdf", height = 10, width = 10)
ggplot(hq.metadata, aes(Kraken_contamination, Contamination))+
geom_point(aes(fill=Rank), shape=21)+
scale_x_sqrt(labels=comma)+
scale_y_sqrt(labels=comma)+
#scale_shape_manual(values = c(0,1,2,3,4,5,6,8))+
