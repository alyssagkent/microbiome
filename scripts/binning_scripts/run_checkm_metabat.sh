#$ -S /bin/bash
#$ -N checkm
#$ -e /workdir/users/agk85/CDC2/logs/metabat_checkm_$JOB_ID.err
#$ -o /workdir/users/agk85/CDC2/logs/metabat_checkm_$JOB_ID.out
#$ -t 38
#$ -V
...
WRK2=/workdir/agk85
FOLDER=$WRK2/metabat/${NAME}
OUT=$WRK2/metabat/${NAME}/checkm_lineage

 if [ ! -d $OUT ]; then mkdir -p $OUT; fi

LOG=$WRK/logs/metabat_checkm_$JOB_ID_${NAME}.out

echo start checkm ${NAME} `date` >> $LOG 2>&1

#run checkm
source  /programs/checkm_161011/init
checkm lineage_wf -t 8 -x fa /workdir/agk85/metabat/${NAME} $OUT
