...
g = Graph.Read_Ncol('/workdir/users/agk85/CDC/newhic/mapping_mgm/trans_primary_ncol_' + '98' + '_sub_withexcise_noeuks.txt',weights=True, directed=True)
...
outhandle = '{0}/arg_v_org/metagenomes3/mgm_sub_histograms/arg_org_mgm_separated_{1}_{2}_{3}_{4}.tbl'.format('/workdir/users/agk85/CDC' , '95' , '98' ,str(args.depth), str(2))
...
for pat in patients:
	for i in range(len(delims)):
		orghisthandle = '/workdir/users/agk85/CDC/arg_v_org/metagenomes3/mgm_sub_histograms/org_argcounts_'+ pat + '_' + delim_names[i] + str(args.depth) + '.txt'
		arghisthandle = '/workdir/users/agk85/CDC/arg_v_org/metagenomes3/mgm_sub_histograms/arg_orgcounts_'+ pat + '_' + delim_names[i] + str(args.depth) + '.txt'
		delim = delims[i]
		orghistcount = {}
		arghistcount = {}
		level_orgs = []
...
for i in range(len(delims)):
	orghisthandle = '/workdir/users/agk85/CDC/arg_v_org/metagenomes3/mgm_sub_histograms/org_argcounts_'+  delim_names[i] + str(args.depth) + '.txt'
	arghisthandle = '/workdir/users/agk85/CDC/arg_v_org/metagenomes3/mgm_sub_histograms/arg_orgcounts_'+  delim_names[i] + str(args.depth) + '.txt'
	delim = delims[i]
	orghistcount = {}
	arghistcount = {}
	level_orgs = []
