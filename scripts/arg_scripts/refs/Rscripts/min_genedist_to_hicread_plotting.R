...
setwd("/workdir/users/agk85/CDC/arg_v_org/metagenomes3/min_dist")
infile <- read.csv("argdist_to_nearest_hicread.txt",sep="\t", header=T)
...
pdf("Min_dist_vs_scaffold_density.pdf")
...
pdf("Min_dist_vs_scaffold_density_loglog.pdf")
ggplot(infile, aes(x = Scaffold_length,y =Minimum_distance))+
stat_binhex()+
...
pdf("Min_dist_gene_nogroups_ratio.pdf")
ggplot(data=sub)+
...
pdf("Min_dist_gene_groupcolor_distance.pdf")
...
pdf("Min_dist_gene_groupcolor_distance_inset_0_100000.pdf")
ggplot(data=sub, aes(fill=groups))+
...
pdf("Min_dist_gene_groupcolor_distance_inset_0_50000.pdf")
ggplot(data=sub, aes(fill=groups))+
geom_histogram(aes(Minimum_distance), position="stack",bins=50)+
...
pdf("Min_dist_gene_groupcolor_distance_inset_0_20000.pdf")
ggplot(data=sub, aes(fill=groups))+
geom_histogram(aes(Minimum_distance), position="stack",bins=50)+
...
pdf("Min_dist_gene_groupcolor_ratio.pdf")
ggplot(data=sub, aes(fill=groups))+
...
pdf("Min_dist_gene_groupcolor_distance_loglog.pdf")
ggplot(data=sub, aes(fill=groups))+
geom_histogram(aes(Minimum_distance), position="stack",bins=50)+
scale_x_log10(labels=comma)+
ylab("Number of Gene-Taxonomy Combinations")
...
pdf("Min_dist_gene_groups_distance.pdf",height=7, width=20)
ggplot(data=sub)+
...
#normalize it
pdf("Min_dist_gene_groups_ratio.pdf", height=7, width = 20)
ggplot(data=sub)+
...
pdf("Min_dist_argpat_groups_distance.pdf",height=7, width=20)
...
#normalize it
pdf("Min_dist_argpat_ratio.pdf", height=7, width = 20)
ggplot(data=sub.agg)+
geom_histogram(aes(Ratio))+
xlab("Ratio (Min. Distance to Nearest Read/Scaffold Length)")+
scale_x_continuous(labels=comma)+
...
sub.agg.add <- merge(sub.agg, sub.trim, by=c("Cluster","Patient","Taxonomy","Minimum_distance"))
sub.agg.add$groups <- cut2(sub.agg.add$Scaffold_length, c(10000,50000,100000,500000,1000000)) 
pdf("Min_dist_argpat_groups_distance_withscftaxa.pdf",height=7, width=20)
ggplot(data=sub.agg.add)+
geom_histogram(aes(Minimum_distance))+
xlab("Min. Distance to Nearest Read")+
facet_grid(.~groups, scales="free")+
...
#normalize it
pdf("Min_dist_argpat_ratio_withscftaxa.pdf", height=7, width = 20)
ggplot(data=sub.agg)+
geom_histogram(aes(Ratio))+
xlab("Ratio (Min. Distance to Nearest Read/Scaffold Length)")+
scale_x_continuous(labels=comma)+
