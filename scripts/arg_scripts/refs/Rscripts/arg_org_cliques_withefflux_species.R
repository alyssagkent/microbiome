...
arginfo= read.csv("arg_v_samp_95_95_names_mech.txt", sep="\t",header = T)
...
connectdf_speciesonly <- subset(connectdf_t, groupvar!="s__.")
...
pdf(paste("heatmap_arg_org_multitaxa_depth1_withefflux_species_none.pdf",sep=""), height=60, width=60,useDingbats=FALSE)
...
pdf(paste("heatmap_arg_org_multitaxa_depth1_withefflux_species_row.pdf",sep=""), height=60, width=60,useDingbats=FALSE)
heatmap.3(t(subdata), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none",
...
pdf(paste("heatmap_arg_org_multitaxa_depth1_withefflux_species_both.pdf",sep=""), height=60, width=60,useDingbats=FALSE)
heatmap.3(t(subdata), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none",
...
connectdf_multipat_speciesonly <- subset(connectdf_multipat_t, groupvar!="s__.")
...
pdf(paste("heatmap_arg_org_",i+1,"+patients_depth1_withefflux_species_none.pdf",sep=""), height=60, width=40,useDingbats=FALSE)
...
dev.off()
pdf(paste("heatmap_arg_org_",i+1,"+patients_depth1_withefflux_species_row.pdf",sep=""), height=60, width=40,useDingbats=FALSE)
heatmap.3(t(subdata_multipat), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none",
	dendrogram="row",	Rowv=T,Colv=F,key=F,symbreaks=FALSE, margins=c(30,30),
	symkey=F, density.info="none", trace="none", labCol=groupvar_ending,
	labRow=paste("Cluster",sub7_multipat_speciesonly$Cluster,sub7_multipat_speciesonly$ARG_name,sep="_"),	cexCol=.8,
...
legend("topright",cex=1.75,title="Family",legend=as.character(d$Taxa),fill=as.character(d$Taxacolor))
dev.off()
pdf(paste("heatmap_arg_org_",i+1,"+patients_depth1_withefflux_species_both.pdf",sep=""), height=60, width=40,useDingbats=FALSE)
heatmap.3(t(subdata_multipat), hclustfun=myclust, distfun=mydist, na.rm = TRUE, scale="none",
	dendrogram="both",	Rowv=T,Colv=T,key=F,symbreaks=FALSE, margins=c(30,30),
	symkey=F, density.info="none", trace="none", labCol=groupvar_ending,
	labRow=paste("Cluster",sub7_multipat_speciesonly$Cluster,sub7_multipat_speciesonly$ARG_name,sep="_"),	cexCol=.8,
...
#subset to a patient
#iterate through taxa 140 X 140 matrix
#
...
for (i in seq(length(pats))){
	...
	sub9$Cluster<-NULL
	sub10<-as.data.frame(t(sub9[,0:(ncol(sub9)-1)]))
	sub11 <- sub10[rowSums(sub10)>0,colSums(sub10)>1]
	k <- create.N.matrix(sub11)
	rownames(k)<- rownames(sub11)
	colnames(k)<- rownames(sub11)
	k[is.na(k)]<-0
	pat_mats[[i]]<-k
}
...
#so with this empty graph, combine it with each of the patients in order to plot
#####goal plot the network one patient after another
#use different sizes small to big and different edge colors for each patient
...
g_simp_species <- delete_vertices(g_simp, V(g_simp)[species=='s__.'])
g_simp_species
...
pecies_simp_noedges_forgephi<-g_simp_species_noedges
...
write.csv(a, "Taxa_issues_withefflux_species.csv")
...
pdf("Org_org_network_circle_phylum_separated_withefflux_species_depth1.pdf",height=50,width=50)
...
for (i in seq(length(pats))){
	...
	pdf(paste("Org_org_network_circle_g",i,"_phylum_withefflux_enterolines_species_depth1.pdf",sep=""),height=50,width=50)
	#1-B314, 2-B316, 3-B320, 4-B331, 5-B335, 6-B357, 7-B370
	vsize=2
...
for (i in seq(length(pats))){
	...
	V(patient_graph)$phylumcolor2 <- V(patient_graph )$phylumcolor
	V(patient_graph)$phylumcolor2[!V(patient_graph)$present]<-NA
	pdf(paste("Org_org_network_circle_g",i,"_phylum_withefflux_species_depth1.pdf",sep=""),height=50,width=50)
	#1-B314, 2-B316, 3-B320, 4-B331, 5-B335, 6-B357, 7-B370
	vsize=2
	plot(patient_graph ,layout=l,vertex.label=NA,vertex.frame.color=NA,vertex.size=vsize,vertex.color=V(patient_graph)$phylumcolor2,edge.color=NA,edge.width=E(patient_graph)$weight_2/line_scaling)
	par(new=TRUE)
...
pdf(paste("Org_org_network_circle_all_phylum_withefflux_species_depth1.pdf",sep=""),height=50,width=50)
vsize=2
...
pdf(paste("Org_org_network_circle_all_phylum_withefflux_enterolines_species_depth1.pdf",sep=""),height=50,width=50)
vsize=2
plot(graph.to.plot,layout=l,vertex.label=NA,vertex.frame.color=V(graph.to.plot)$enterocol,vertex.size=vsize,vertex.color=V(graph.to.plot)$phylumcolor)
par(new=TRUE)
...
pdf(paste("Org_org_network_",layoutname,"_all_phylum_withefflux_enterolines_species_depth1.pdf",sep=""),height=50,width=50)
plot(graph.to.plot,layout=l,vertex.label=NA,vertex.frame.color=V(graph.to.plot)$enterocol,vertex.size=vsize,vertex.color=V(graph.to.plot)$phylumcolor)
par(new=TRUE)
plot(g_simp_species,layout=l,vertex.label=NA,vertex.frame.color=NA,vertex.size=vsize,vertex.color=NA,edge.width=E(g_simp_species)$weight/line_scaling, edge.color=E(g_simp_species)$enterocol)
legend("bottomright",cex = 4,pch=21,legend=c(pp$Phylum),col=c(pp$Phylumcolor),pt.bg=c(pp$Phylumcolor))
...
for (i in seq(length(pats))){
	patient_graph = patient_graphs[[i]]
	print(pats[i])
	pdf(paste("Org_org_network_", layoutname, "_g",i,"_phylum_withefflux_species_depth1.pdf",sep=""),height=50,width=50)
	plot(graph.to.plot,layout=l,vertex.label=NA,vertex.frame.color=V(graph.to.plot)$enterocol,vertex.size=vsize,vertex.color=V(graph.to.plot)$phylumcolor)
	par(new=TRUE)
...
################
pdf(paste("Org_org_network_",layoutname,"_separated_phylum_withefflux_species_depth1.pdf",sep=""),height=50,width=50)
plot(g1,layout=l,vertex.label=NA,vertex.frame.color=V(g1)$enterocol,vertex.size=vsize,vertex.color=V(g1)$phylumcolor,edge.color=kewl[1],edge.width=E(g1)$weight_2/line_scaling)
par(new=TRUE)
plot(g2,layout=l,vertex.label=NA,vertex.frame.color=NA,vertex.size=vsize,vertex.color=NA,edge.color=kewl[2],edge.width=E(g2)$weight_2/line_scaling)
par(new=TRUE)
...
pdf(paste("heatmap_org_org_cliques_depth1_none_all_withefflux_species.pdf",sep=""), height=50, width=50,useDingbats=FALSE)
