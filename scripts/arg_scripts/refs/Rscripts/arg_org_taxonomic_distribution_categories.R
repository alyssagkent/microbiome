...
#make sure you recheck to make sure it is plotting across mechanisms correctly
...
df.arg.agg.genus <-aggregate(value~patients+Cluster+genus+genusname,data=df_melt.pos,sum)
#make binary so patients aren't contributing more because of time-points
df.arg.agg.genus$value <- (df.arg.agg.genus$value>0)+0
colnames(df.arg.agg.genus)<-c("Patient","Gluster","Genus","Genusname","Count")
#aggregate keeping  patients, and taxa and summing over the args
df.arg.agg.genus.reduced <-aggregate(Count ~ Patient + Genus+Genusname, data =  df.arg.agg.genus, sum)
colnames(df.arg.agg.genus.reduced)<-c("Patient","Genus","Genusname","Genus_count")

#SPECIES aggregate keeping clusters, patients, and taxa
df.arg.agg.species <-aggregate(value~patients+Cluster+species+speciesname,data=df_melt.pos,sum)
#make binary so patients aren't contributing more because of time-points
df.arg.agg.species$value <- (df.arg.agg.species$value>0)+0
colnames(df.arg.agg.species)<-c("Patient","Cluster","Species","Speciesname","Count")
#aggregate keeping  patients, and taxa and summing over the args
df.arg.agg.species.reduced <-aggregate(Count ~ Patient + Species+Speciesname, data =  df.arg.agg.species, sum)
colnames(df.arg.agg.species.reduced)<-c("Patient","Species","Speciesname","Species_count")
...
#species
#aggregate keeping clusters, patients, and taxa
df.arg.agg.species <-aggregate(value ~patients+Cluster+Mergedname+Category+species+speciesname, data =  df_melt.pos, sum)
df.arg.agg.species$value <- (df.arg.agg.species$value>0) + 0
colnames(df.arg.agg.species)<-c("Patient","Cluster","Mergedname","Category","Species", "Speciesname","Count")
df.arg.agg.species.reduced<-aggregate(Count~Patient+Species+Speciesname,data=df.arg.agg.species,sum)
