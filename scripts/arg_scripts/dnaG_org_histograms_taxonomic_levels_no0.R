...
delimname = delims[i]
print(delimname)
indata0 = read.csv(paste("dnaG_orgcounts_lumped_", patient,"99_0_",delimname,"0.txt",sep=""), sep="\t",  header=F)
indata1 = read.csv(paste("dnaG_orgcounts_lumped_",patient,"99_0_",delimname,"1.txt",sep=""), sep="\t", row.names=1, header=F)
indata2 = read.csv(paste("dnaG_orgcounts_lumped_",patient,"99_0_",delimname,"2.txt",sep=""), sep="\t", row.names=1, header=F)
...
pdf(paste("All_dnaG_ORG_histogram_no0_lumped_",delimname,"_with2.pdf",sep=""), height = 20, width =15)
...
colnames(m)<-header
rownames(m)<-patients
write.table(m, paste("dnaG_histogram_",delimname, "_stats_lumped.txt",sep=""),quote = FALSE,sep ="\t")
}
