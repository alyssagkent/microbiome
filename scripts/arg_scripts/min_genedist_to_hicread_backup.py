	...
	with open(cvb) as infile:
		...
		for line in infile:
			...
			if (quality != 'NA' and reads>=minreads):
				...
				if (taxonomy != 'k__; p__; c__; o__; f__; g__; s__;' and genes != ['NA'] and taxonomy != 'NA'):
					for gene in genes:
							...
							for contig2 in contigdict[contig1]:
								...
								if contig2 in bindict[binid]:
									try:
										...
										argdict[gene][taxonomy] = [contig2]
						except KeyError:
							a = 1
							#this contig doesn't have any hic reads	
			...
			try:
				...
				if len(gene_taxonomies)>0:
					for taxa in gene_taxonomies:
						...
						if (scf in taxascfs or taxascfs == []):
							in_genescf = '1'
							minimum_distance = 0
						else:
							in_genescf = '0'
				...
				a = 1
				#did not pass a thresh or something
