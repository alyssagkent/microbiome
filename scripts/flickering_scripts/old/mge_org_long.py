...
def get_code(gene, org, samp):
	...
	color = '6'
	try:
		connection = str(geneorgdict[samp][gene][org])
	except KeyError:
		connection = '0'
	try:
		genepres = str(genedict[samp][gene])
	except KeyError:
		genepres = '0'
...
outhandle = '/workdir/users/agk85/CDC2/bins/flickering2/{0}_org_hic_separated_{1}_{2}.tbl'.format(args.genetype, level, str(args.minreads))
