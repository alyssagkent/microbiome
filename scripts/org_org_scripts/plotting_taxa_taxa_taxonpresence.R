...
patient	genetype	minthresh	consistency
		...
		g1 <- simplify( g, remove.multiple=T, edge.attr.comb=c(weight="sum"))
		d = edge_density(g1)
		densities[i] = d
		if (patient == 'all'){
...
ent <- V(gtp)$name %in% g1_taxa_df$taxaevels = c('k__','p__','c__','o__','f__','g__', 's__')
