...
level = 'all'
outhandle = '/workdir/users/agk85/CDC2/bins/connections_{0}_org_{1}_{2}.txt'.format(args.genetype, level, str(args.minreads))

with open(outhandle,'w') as outfile:
	header = 'patient\tsample\tgenetype\tminthresh\tgeneid\ttaxonomy\n'
	outfile.write(header)
	for sample in samples:
		patient = sample.split('-')[0]
		connections = connect_dict[sample]
		for connection in connections:
			geneid = connection[0]
			taxonomy = connection[1]
			outfile.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\n'.format(patient,sample,args.genetype,args.minreads,geneid, taxonomy))
