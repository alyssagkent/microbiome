...
with open(args.connections) as infile:
	...
	for line in infile:
		...
		for level in levels:
			taxa = taxonomy.split(level)[1].split(';')[0]
			if taxa != '':
				connection_dict[sample][level].append((geneid,taxa))
...
with open('/workdir/users/agk85/CDC2/bins/all_das_1_contigs_v_bins_all.txt') as dasfile:
	...
	for line in dasfile:
		count,sample,contig,binid,bin_length,quality,association_type,total_count,transcount,norm_l,norm_rf,contig_taxonomy,kraken_taxonomy,gtdb_taxonomy,arg_presence,arg_clusters,arg_genes,mge_presence,mge_clusters,mge_genes= line.strip().split('\t')
		trans_count = float(transcount)
		for level in levels:
			if kraken_taxonomy != 'NA':
...
with open(outhandle, 'w') as outfile, open(outhandle2,'w') as outfile2:
	...
	headerfull = 'Patient\tT1\tT2\tLevel\tTaxa1\tTaxa1_t1_abund\tTaxa2\tTaxa1_t2_abund\tTaxa2_t1_abund\tTaxa2_t2_abund\tGeneid\tGene_name\tGene_contig\tGene_t1_abund\tGene_t2_abund\tTrans_taxa2_t1\tTrans_taxa2_t2\tNumber_non_t1_tps\tOther_tps\t\tContig_taxa_connection\tContig_taxa\n'
				...
				if patient1 == patient2:
					...
					if timepoint_dict[t1] == '1':
						for connection in gainlist:
							...
							try:
								trans_hic_t1 = gene_taxa_trans[t1][level][gene][taxa2]
							except KeyError:
								print(t1,gene,taxa2)
								trans_hic_t1 = 0
							trans_hic_t2 = gene_taxa_trans[t2][level][gene][taxa2]
							num_nont1 = number_nont1
							other_tps = ','.join(nont1)
							genecontigs = ','.join(set(gene_contig[t2][level][gene][taxa2]))
							contig_taxa_connection = t1contigpres
							contigtaxa1_t1 = 'contigtaxa_'+','.join(t1contigtaxa)
							outfile2.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\t{13}\t{14}\t{15}\t{16}\t{17}\t{18}\t{19}\t{20}\n'.format(patient1,t1,t2,level,taxa1,taxa1_t1_abund,taxa1_t2_abund,taxa2,taxa2_t1_abund,taxa2_t2_abund, gene,genename,t1rpkm,genecontigs, t2rpkm,trans_hic_t1,trans_hic_t2,num_nont1,other_tps,contig_taxa_connection,contigtaxa1_t1))
