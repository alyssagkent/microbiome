...
tbldict = {}
tblpaths = glob.glob(base + '/combo_tables/metagenomes/B*_master_scf_table.txt')
...
organisms = []
organisms2 = []
hicpaths = glob.glob(base + '/hicpro/output/*/hic_results/data/*/B*_trans_primary_' + pidthresh + '_noeuks.txt')
...
outhandle = base + '/hicpro/figures/B_org_v_org_' + pidthresh + '.txt'
mismatchhandle = base + '/hicpro/figures/B_org_v_org_' + pidthresh + '_mismatches_variablecount.txt'
...
header = header + '\n'
outhandle = base + '/hicpro/figures/B_org_v_org_' + pidthresh + '_mge.txt'
with open(outhandle,'w') as outfile:
	outfile.write(header)
	for org1 in orglist:
		outfile.write(org1)
