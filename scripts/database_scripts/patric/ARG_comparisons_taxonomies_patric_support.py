...
bad_taxids = []
inhandle = '/workdir/users/agk85/CDC2/args/patric_comparisons/args_99_nr_1e-100.out'
gene_taxonomies = {}
...
genedict = {}
noblastresult = []
comparehandle = '/workdir/users/agk85/CDC2/bins/patric_figures/arg_' + minreads + '_base_hic_taxonomies.txt'
...
with open(comparehandle) as cfile:
	...
	for line in cfile:
		...
		hic_taxonomies = list(set(hicsupport_taxonomies + hic_taxonomies))
...
#delims = ['s__']
outhandle = '/workdir/users/agk85/CDC2/bins/patric_figures/arg_' + minreads + '_support_patric_distribution.txt'
...
patrichandle = '/workdir/users/agk85/CDC2/args/patric_comparisons/Patric_arg_taxonomies.txt'
hichandle = '/workdir/users/agk85/CDC2/args/patric_comparisons/Hic_arg_' + minreads + '_taxonomies.txt'
basehandle = '/workdir/users/agk85/CDC2/args/patric_comparisons/Base_arg_' + minreads + '_taxonomies.txt'
with open(patrichandle,'w') as patricfile, open(hichandle, 'w') as hicfile, open(basehandle,'w') as basefile:
	for cluster in cluster_ncbitaxa.keys():
		taxonomies = list(set(cluster_ncbitaxa[cluster]))
		for taxonomy in taxonomies:
			patricfile.write('{0}\t{1}\n'.format(cluster, taxonomy))
	for cluster in cluster_hictaxa.keys():
		taxonomies = list(set(cluster_hictaxa[cluster]))
		for taxonomy in taxonomies:
			hicfile.write('{0}\t{1}\n'.format(cluster, taxonomy))
	for cluster in cluster_basetaxa.keys():
		taxonomies = list(set(cluster_basetaxa[cluster]))
		for taxonomy in taxonomies:
			basefile.write('{0}\t{1}\n'.format(cluster, taxonomy))
